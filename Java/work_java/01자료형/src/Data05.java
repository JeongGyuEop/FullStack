...
//		int c = 32768; // 4byte 크기에 저장될 값 32768
		...
		/*
		 * int 자료형 변수 c에 저장된 값(4byte 크기 변수 메모리에 저장된 큰 정수값)을
		 * short 자료형 변수 b(2byte 크기에 정수가 저장될 작은 메모리)에 저장시키려고 할때
		 * 컴파일러가 자동으로 int 자료형 변수 d에 저장된 32768을 
		 * short 자료형 허용값 -32768 ~ 32767으로 자동 형 변환 하지 않기 때문에
		 * 개발자가 강제로 자료형의 형태를 변환하는 작업(캐스팅)을 해야한다.
		 *
		 */
		
		b = (short)c; // 강제 형 변환을 할 경우 값이 범람하게 되어
					  // 엉뚱한 값 -32768이 저장될 수 있으므로 이전에 유의해야 한다
		System.out.println(b);
		...
		System.out.println("doubleValue2 변수 : " + doubleValue2); // 9.78
		System.out.println("intValue2 변수 : " + intValue2); // 9
		
		/*
		 * 요약.
		 * 		자동형변환 : 작은 숫자에서 큰 숫자로 컴파일러가 알아서 변환
		 * 					(예 : int -> double)
		 * 		강제형변환 : 큰 숫자에서 작은 숫자로 개발자가 직업 변환
		 * 					(예 : double -> int)
		 */
