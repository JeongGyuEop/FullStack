...
<%--기본생성자를 호출해 MemberBean클래스의 객체를 생성합니다. --%>
 ...
 <%-- mb.setId(String id)메소드 호출시 매개변수 id로 value속성에 적은 값을 전달해서
      property속성에 적은 id변수에 저장 시킴
   --%>
 <jsp:setProperty name="mb" property="id" value='<%=request.getParameter("id")%>'/>
 <jsp:setProperty name="mb" property="pwd" value='<%=request.getParameter("pwd")%>'/>
...
<!--
	MemberBean클래스의 변수와 setter메소드들 그리고 setter메소드 호출구문 자바코드  
	private String id;        public void setId(String id){ this.id = id; }    memberBean.setId("admin");
	private String pwd;		  public void setPwd(String pwd){ this.pwd = pwd; }  memberBean.setPwd("1234");
	private String name;      public void setName(String name){ this.name = name; }  memberBean.setName("홍길동");
	private String email;     public void setEmail(String email){ this.email = email; }  memberBean.setEmail("admin@naver.com");
...
<%--
	<jsp:setProperty>액션태그 (셋프로퍼티 액션태그)
	- 자바빈 역할을 하는 VO의 setter메소드를 자바코드 대신 작성할 태그입니다
	
	문법
		<jsp:setProperty name="useBean액션태그의 id속성에 작성한 값"  
					     property="setter메소드 호출시 매개변수로 전달해서 저장할 변수명"	
