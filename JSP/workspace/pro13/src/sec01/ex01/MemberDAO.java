...
public class MemberDAO {
	...
	public ArrayList listMembers() {
		...
		try {
			...
			while( rs.next() ) {
				...
				//MemberBean객체를 생성 하여 각 인스턴스변수에 저장시킵니다.
	...
	*/
	...
	// 삭제 <a> 링크를 클릭했을 떄... 서블릿으로 전송한 삭제할 회원 아이디를
	// 다시~ delMember 메소드의 메개변수로 전달받아서
	// DELETE 문장을 완성 후 ~ DB의 t_member 테이블에 저장된 회원 한사람의 정보를
	// 삭제시키는 기능의 메소드
	public void delMemebr(String id) {
		
		try {
			// 순서 1. 커넥션 풀(DataSource) 에서
			//         미리 DB와 연결을 맺은 클라이언트의 접속 정보를 가지고 있는
			// 	  	   Connection 객체 하나 빌려와서 DB와 MemberDAO와 연결
			con = dataSource.getConnection();
			
			// 순서 2. 위 String id 매개변수로 전달받은 삭제할 회우너 아이디와 일치하는
			//         회원레코드(행) 삭제하는 DELETE 문 작성
			String query = "delete from t_member where id=?";
			
			
			// 순서 3. query 변수에 저장된 전체 DELETE 문자열을 미리 로드한 
			//         OraclePreparedStatementWrapper 실행 객체 얻기
			pstmt = con.prepareStatement(query);
			
			// 순서 3-1. OraclePreparedStatementWrapper 실행 객체에 미리 로드된 
			//			 delete 문장 중에 ? 기호 대신 매개변수로 전달받은
			//           삭제할 회원 아이디로 설정
			// 요약 : ? 설정
			pstmt.setString(1, id);
			
			// 순서 4. OraclePreparedStatementWrapper 실행 객체에 설정된
			//		   delete form t_member where id=hong 전체 문장을
			//		   DB의 t_member 테이블에 전송해서 실행!
			pstmt.executeUpdate(); // delete에 성공하면 성공한 레코드(행)갯수 반환
								   // 실패하면 0 반환
			
			
		} catch (Exception e) {
			System.out.println("MemberDAO에 delMember 메소드 내부에서"
					+ " DELETE SQL 문 실행 오류 : " + e);
		} finally {
			// 자원해제 구문
