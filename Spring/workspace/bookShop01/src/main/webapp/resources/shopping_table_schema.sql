...
REM INSERTING into T_GOODS_DETAIL_IMAGE
    -- 부분은 SQL 주석(comment)입니다. SQL에서는 -- 또는 /* /를 사용하여 주석을 작성할 수 있습니다. 
    -- 위  코드는 REM 키워드를 사용하여 주석을 작성한 것으로, 이는 SQLPlus에서 사용될 때 표시됩니다.   
   
-- SET DEFINE OFF;
    -- 이 부분은 SQL*Plus를 사용할 때 유용한 명령어입니다. 
    -- 이 명령어를 사용하면 & 문자가 SQL 명령문에서 변수를 나타내는 기호로 인식되지 않습니다.
    -- 예를 들어, &VARIABLE_NAME을 사용하여 변수를 나타내는 경우, 
    -- 이를 SQL*Plus에서 사용할 경우 &가 변수 값으로 대체됩니다. 
    -- 하지만 SET DEFINE OFF;를 사용하면 & 문자가 그대로 인식되므로, 
    -- SQL 명령문에서 & 문자를 사용할 수 있습니다.   
