...
$(function () {
  ...
  window.mySwipe = $('#mySwipe')
    ...
    .data('Swipe');
  // .data('Swipe') 는 Swipe객체의 메서드로 생성된 객체를 반환하여
  ...
  $('.playBtn').on('click', function () {
    //setTimeout()메소드를 clearTimeout()메소드로 제거한후
  ...
  $('#tabmenu dt a').on('mouseover focus click', function () {
    ...
    //3개의 이벤트(마우스오버, 포커스, 클릭)중 하나의 이벤트가 발생한 영역을
  ...
  //조건
  //만약 "pop"쿠키명과 함께 "no"쿠키값이 웹브라우저에 저장되어 있지 않으면?
  if ($.cookie('pop') != 'no') {
    $('#pop_wrap').show();
  }
